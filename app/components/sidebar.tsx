...
export default function Sidebar({ roleId }: SidebarProps) {
    ...
    const menuItems: MenuProps["items"] = [
        ...
        roleId === 1 && {
            ...
            children: [
                { key: "user", label: <Link href="/user-management/user">{t.user}</Link> },
                { key: "role", label: <Link href="/user-management/role">{t.role}</Link> },
            ],
        },
        roleId === 1 && {
            key: "reports",
        ...
        >
            <div className="p-6 flex flex-col justify-between h-full">
                ...
                <Button
                    ...
                    type="primary"
                    danger
                    className="flex items-center justify-center gap-2 w-full mt-6 flex-shrink-0"
                    icon={<LogoutOutlined />}
                >
                    {!collapsed && t.logout}
                </Button>
